...
if(isset($_GET["eliminar"])){
    $conexion->query("DELETE FROM `docente` WHERE id_docente='".$_GET["eliminar"]."'");
    header("location: docente.php");
    exit();
}
?>
                <div class="container-fluid">
                    <h1 class="h3 mb-4 text-gray-800">Gestión Docente</h1>
                    <div class="card mb-4">
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                            <h6 class="m-0 font-weight-bold text-primary">Lista Docentes</h6>
                        ...
                        <div class="card-body">
                            ...
                            <table class="table">
                                ...
                                <?php
                                    $muestro_docentes = $conexion->query("SELECT id_docente,CONCAT(apellidos,' ',nombres) AS nombres FROM docente");
                            ...
                            <?php } elseif(!isset($_GET["nuevo"])&&isset($_GET["editar"])) {
                                if(isset($_POST["editar"])){
                                    if(!empty($_POST["editar"])){
                                        $datos = $_POST["editar"];
                                        $conexion->query("UPDATE `docente` SET `id_docente`='$datos[0]',`clave`='$datos[1]',`nombres`='$datos[2]',`apellidos`='$datos[3]',`id_estado`='$datos[4]' WHERE id_docente='".$_GET["editar"]."'");
                                        header("location: docente.php");
                                        exit();
                                    }
                                }
                                $muestro_datos = $conexion->query("SELECT `id_docente`, `clave`, `nombres`, `apellidos`, `id_estado` FROM `docente` WHERE id_docente='".$_GET["editar"]."'");
                            ...
                            <?php } elseif(isset($_GET["nuevo"])&&!isset($_GET["editar"])) {
                                if(isset($_POST["crear"])){
                                    if(!empty($_POST["crear"])){
                                        $datos = $_POST["crear"];
                                        $conexion->query("INSERT INTO `docente`(`id_docente`, `clave`, `nombres`, `apellidos`, `id_estado`) VALUES ('$datos[0]','$datos[1]','$datos[2]','$datos[3]','$datos[4]')");
                                        header("location: docente.php");
                                        exit();
                                    }
                                }
